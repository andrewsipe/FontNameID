#!/usr/bin/env python3
"""
Font NameID 12 Replacer Script

Replaces the nameID="12" (Designer URL) record in font files.
...
def process_ttx_file(filepath, designer_url, string_override=None, dry_run=False):
    """Process TTX (XML) file to replace nameID="12" record"""
    try:
        if LXML_AVAILABLE:
        ...
        # Priority: string_override → designer_url
        new_name = string_override if string_override else designer_url

        # Find namerecord with nameID="12"
        namerecord_12 = name_table.find(
            './/namerecord[@nameID="12"][@platformID="3"][@platEncID="1"][@langID="0x409"]'
        )

        file_changed = False
        if namerecord_12 is not None:
            # Capture old value before updating
            old_text = namerecord_12.text.strip() if namerecord_12.text else ""
            if old_text == new_name:
                show_unchanged(12, filepath, old_text, dry_run, console)
            else:
                # Format text with proper indentation (newline + 6 spaces)
                namerecord_12.text = f"\n      {new_name}\n    "
                file_changed = True
                show_updated(12, filepath, old_text, new_name, dry_run, console)
        else:
            ...
            file_changed = True
            show_created(12, filepath, new_name, dry_run, console)
        ...
        if file_changed:
            ...
            deduplicate_namerecords_ttx(name_table, 12)
...
def process_binary_font(filepath, designer_url, string_override=None, dry_run=False):
    ...
    try:
        ...
        # Look for existing nameID=12 record with the specific platform/encoding
        found = False
        file_changed = False
        ...
        if file_changed:
            ...
            deduplicate_namerecords_binary(name_table, 12)
...
def process_file(filepath, designer_url, string_override=None, dry_run=False):
    ...
    if ext == ".ttx":
        return process_ttx_file(filepath, designer_url, string_override, dry_run)
    else:
        return process_binary_font(filepath, designer_url, string_override, dry_run)


def process_files(file_paths, script_args, batch_context=False):
    """
    Core processing logic for NameID12Replacer.
    ...
    if script_args.string:
        operations.append(
            f"Replace nameID 12 (Designer URL) with exact string: '{script_args.string}'"
        )
    else:
        designer_url = script_args.designer_url or "user input"
        operations.append(f"Replace nameID 12 (Designer URL) with: {designer_url}")

    # Define the file processing function for this script
    def process_single_file(filepath, args, dry_run, stats=None):
        return process_file(filepath, args.designer_url, args.string, dry_run)
...
# Flag mapping for explicit syntax (--id12:flagname=value)
SCRIPT_FLAG_MAP = {
    "designer_url": "-du",
    "designer-url": "-du",
    "designerurl": "-du",
...
def main():
    ...
    sys.argv = _preprocess_explicit_syntax(sys.argv, 12)

    parser = argparse.ArgumentParser(
        description="Replace nameID='12' (Designer URL) records in font files",
    ...
    parser.add_argument(
        "-du",
        "--designer-url",
        default="https://designerurl.com",
        help="Designer URL (default: 'https://designerurl.com')",
    )

    parser.add_argument(
        "-str",
        "--string",
        help="Override nameID=12 content with exact string (supersedes all other options)",
    )
