#!/usr/bin/env python3
"""
Font NameID 13 Replacer Script

Replaces the nameID="13" (License Description) record in font files.
...
def process_ttx_file(filepath, license_text, string_override=None, dry_run=False):
    """Process TTX (XML) file to replace nameID="13" record"""
    try:
        if LXML_AVAILABLE:
        ...
        # Priority: string_override → license_text
        new_name = string_override if string_override else license_text

        # Find namerecord with nameID="13"
        namerecord_13 = name_table.find(
            './/namerecord[@nameID="13"][@platformID="3"][@platEncID="1"][@langID="0x409"]'
        )

        file_changed = False
        if namerecord_13 is not None:
            # Capture old value before updating
            old_text = namerecord_13.text.strip() if namerecord_13.text else ""
            if old_text == new_name:
                show_unchanged(13, filepath, old_text, dry_run, console)
            else:
                # Format text with proper indentation (newline + 6 spaces)
                namerecord_13.text = f"\n      {new_name}\n    "
                file_changed = True
                show_updated(13, filepath, old_text, new_name, dry_run, console)
        else:
            ...
            file_changed = True
            show_created(13, filepath, new_name, dry_run, console)
        ...
        if file_changed:
            ...
            deduplicate_namerecords_ttx(name_table, 13)
...
def process_binary_font(filepath, license_text, string_override=None, dry_run=False):
    ...
    try:
        ...
        # Look for existing nameID=13 record with the specific platform/encoding
        found = False
        file_changed = False
        ...
        if file_changed:
            ...
            deduplicate_namerecords_binary(name_table, 13)
...
def process_file(filepath, license_text, string_override=None, dry_run=False):
    ...
    if ext == ".ttx":
        return process_ttx_file(filepath, license_text, string_override, dry_run)
    else:
        return process_binary_font(filepath, license_text, string_override, dry_run)


def process_files(file_paths, script_args, batch_context=False):
    """
    Core processing logic for NameID13Replacer.
    ...
    if script_args.string:
        operations.append(
            f"Replace nameID 13 (License) with exact string: '{script_args.string}'"
        )
    else:
        license_text = script_args.license or "user input"
        operations.append(f"Replace nameID 13 (License) with: {license_text}")

    # Define the file processing function for this script
    def process_single_file(filepath, args, dry_run, stats=None):
        return process_file(filepath, args.license, args.string, dry_run)
...
# Flag mapping for explicit syntax (--id13:flagname=value)
SCRIPT_FLAG_MAP = {"license": "-l", "l": "-l", "string": "-str", "str": "-str"}
...
def main():
    ...
    sys.argv = _preprocess_explicit_syntax(sys.argv, 13)

    parser = argparse.ArgumentParser(
        description="Replace nameID='13' (License Description) records in font files",
    ...
    parser.add_argument(
        ...
        "--license",
        default="license",
        help="License description text (default: 'license')",
    )

    parser.add_argument(
        "-str",
        "--string",
        help="Override nameID=13 content with exact string (supersedes all other options)",
    )
