...
def process_file(
    ...
    if ext == ".ttx":
        return process_ttx_file(
            filepath, license_text, string_override, dry_run, empty_fields_only
        )
    else:
...
def process_files(file_paths, script_args, batch_context=False):
    ...
    if getattr(script_args, "empty_fields_only", False):
        operations.append("Only fill blank nameID 13 entries (--empty-fields-only)")
...
def main():
    ...
    parser.add_argument(
        ...
        help="Only fill blank Windows English nameID 13; do not overwrite existing text",
    )
...
class NameID13Replacer:
    ...
    supported_flags = {"license", "string", "empty_fields_only"}
