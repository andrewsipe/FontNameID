#!/usr/bin/env python3
"""
Font NameID 14 Replacer Script

Replaces the nameID="14" (License URL) record in font files.
...
def process_ttx_file(filepath, license_url, string_override=None, dry_run=False):
    """Process TTX (XML) file to replace nameID="14" record"""
    try:
        if LXML_AVAILABLE:
        ...
        # Priority: string_override → license_url
        new_name = string_override if string_override else license_url

        # Find namerecord with nameID="14"
        namerecord_14 = name_table.find(
            './/namerecord[@nameID="14"][@platformID="3"][@platEncID="1"][@langID="0x409"]'
        )

        file_changed = False
        if namerecord_14 is not None:
            # Capture old value before updating
            old_text = namerecord_14.text.strip() if namerecord_14.text else ""
            if old_text == new_name:
                show_unchanged(14, filepath, old_text, dry_run, console)
            else:
                # Format text with proper indentation (newline + 6 spaces)
                namerecord_14.text = f"\n      {new_name}\n    "
                file_changed = True
                show_updated(14, filepath, old_text, new_name, dry_run, console)
        else:
            ...
            file_changed = True
            show_created(14, filepath, new_name, dry_run, console)
        ...
        if file_changed:
            ...
            deduplicate_namerecords_ttx(name_table, 14)
...
def process_binary_font(filepath, license_url, string_override=None, dry_run=False):
    ...
    try:
        ...
        # Look for existing nameID=14 record with the specific platform/encoding
        found = False
        file_changed = False
        ...
        if file_changed:
            ...
            deduplicate_namerecords_binary(name_table, 14)
...
def process_file(filepath, license_url, string_override=None, dry_run=False):
    ...
    if ext == ".ttx":
        return process_ttx_file(filepath, license_url, string_override, dry_run)
    else:
        return process_binary_font(filepath, license_url, string_override, dry_run)


def process_files(file_paths, script_args, batch_context=False):
    """
    Core processing logic for NameID14Replacer.
    ...
    if script_args.string:
        operations.append(
            f"Replace nameID 14 (License URL) with exact string: '{script_args.string}'"
        )
    else:
        license_url = script_args.license_url or "user input"
        operations.append(f"Replace nameID 14 (License URL) with: {license_url}")

    # Define the file processing function for this script
    def process_single_file(filepath, args, dry_run, stats=None):
        return process_file(filepath, args.license_url, args.string, dry_run)
...
# Flag mapping for explicit syntax (--id14:flagname=value)
SCRIPT_FLAG_MAP = {
    "license_url": "-lu",
    "license-url": "-lu",
    "licenseurl": "-lu",
...
def main():
    ...
    sys.argv = _preprocess_explicit_syntax(sys.argv, 14)

    parser = argparse.ArgumentParser(
        description="Replace nameID='14' (License URL) records in font files",
    ...
    parser.add_argument(
        "-lu",
        "--license-url",
        default="https://vendorurl.com/license",
        help="License URL (default: 'https://vendorurl.com/license')",
    )

    parser.add_argument(
        "-str",
        "--string",
        help="Override nameID=14 content with exact string (supersedes all other options)",
    )
