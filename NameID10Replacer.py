#!/usr/bin/env python3
"""
Font NameID 10 Replacer Script

Replaces the nameID="10" (Description) record in font files.
...
def process_ttx_file(filepath, description, string_override=None, dry_run=False):
    """Process TTX (XML) file to replace nameID="10" record"""
    try:
        if LXML_AVAILABLE:
        ...
        # Priority: string_override → description
        new_name = string_override if string_override else description

        # Find namerecord with nameID="10"
        namerecord_10 = name_table.find(
            './/namerecord[@nameID="10"][@platformID="3"][@platEncID="1"][@langID="0x409"]'
        )

        file_changed = False
        if namerecord_10 is not None:
            # Capture old value before updating
            old_text = namerecord_10.text.strip() if namerecord_10.text else ""
            if old_text == new_name:
                show_unchanged(10, filepath, old_text, dry_run, console)
            else:
                # Format text with proper indentation (newline + 6 spaces)
                namerecord_10.text = f"\n      {new_name}\n    "
                file_changed = True
                show_updated(10, filepath, old_text, new_name, dry_run, console)
        else:
            ...
            file_changed = True
            show_created(10, filepath, new_name, dry_run, console)
        ...
        if file_changed:
            ...
            deduplicate_namerecords_ttx(name_table, 10)
...
def process_binary_font(filepath, description, string_override=None, dry_run=False):
    ...
    try:
        ...
        # Look for existing nameID=10 record with the specific platform/encoding
        found = False
        file_changed = False
        ...
        if file_changed:
            ...
            deduplicate_namerecords_binary(name_table, 10)
...
def process_file(filepath, description, string_override=None, dry_run=False):
    ...
    if ext == ".ttx":
        return process_ttx_file(filepath, description, string_override, dry_run)
    else:
        return process_binary_font(filepath, description, string_override, dry_run)


def process_files(file_paths, script_args, batch_context=False):
    """
    Core processing logic for NameID10Replacer.
    ...
    if script_args.string:
        operations.append(
            f"Replace nameID 10 (Description) with exact string: '{script_args.string}'"
        )
    else:
        description = script_args.description or "user input"
        operations.append(f"Replace nameID 10 (Description) with: {description}")

    # Define the file processing function for this script
    def process_single_file(filepath, args, dry_run, stats=None):
        return process_file(filepath, args.description, args.string, dry_run)
...
# Flag mapping for explicit syntax (--id10:flagname=value)
SCRIPT_FLAG_MAP = {
    "description": "-desc",
    "desc": "-desc",
    "string": "-str",
...
def main():
    ...
    sys.argv = _preprocess_explicit_syntax(sys.argv, 10)

    parser = argparse.ArgumentParser(
        description="Replace nameID='10' (Description) records in font files",
    ...
    parser.add_argument(
        ...
        "--description",
        default="description",
        help="Description text (default: 'description')",
    )

    parser.add_argument(
        "-str",
        "--string",
        help="Override nameID=10 content with exact string (supersedes all other options)",
    )
