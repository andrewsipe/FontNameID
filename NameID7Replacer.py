...
def process_ttx_file(filepath, family, designer, string_override=None, dry_run=False):
    """Process TTX (XML) file to replace nameID="7" record"""
    try:
        if LXML_AVAILABLE:
        ...
        # Priority: string_override → constructed trademark string
        if string_override:
        ...
        # Find namerecord with nameID="7"
        namerecord_7 = name_table.find(
            './/namerecord[@nameID="7"][@platformID="3"][@platEncID="1"][@langID="0x409"]'
        )

        file_changed = False
        if namerecord_7 is not None:
            # Capture old value before updating
            old_text = namerecord_7.text.strip() if namerecord_7.text else ""
            if old_text == new_name:
                show_unchanged(7, filepath, old_text, dry_run, console)
            else:
                # Format text with proper indentation (newline + 6 spaces)
                namerecord_7.text = f"\n      {new_name}\n    "
                file_changed = True
                show_updated(7, filepath, old_text, new_name, dry_run, console)
        else:
            ...
            file_changed = True
            show_created(7, filepath, new_name, dry_run, console)

        # Only save if changes were made
        if file_changed:
            if not dry_run:
                # Deduplicate and write back without reformatting (preserve existing whitespace)
                deduplicate_namerecords_ttx(name_table, 7)
                if LXML_AVAILABLE:
                    tree.write(
                        filepath,
                        encoding="utf-8",
...
def process_binary_font(
    filepath, family, designer, string_override=None, dry_run=False
):
    ...
    try:
        ...
        # Look for existing nameID=7 record with the specific platform/encoding
        found = False
        file_changed = False
        ...
        if file_changed:
            if not dry_run:
                ...
                deduplicate_namerecords_binary(name_table, 7)
                font.save(filepath)
            show_saved(filepath, dry_run, console)
        font.close()
        return True
...
def process_file(filepath, family, designer, string_override=None, dry_run=False):
    ...
    if ext == ".ttx":
        return process_ttx_file(filepath, family, designer, string_override, dry_run)
    else:
        return process_binary_font(filepath, family, designer, string_override, dry_run)


def process_files(file_paths, script_args, batch_context=False):
    """
    Core processing logic for NameID7Replacer.
    ...
    def process_single_file(filepath, args, dry_run, stats=None):
        return process_file(filepath, args.family, args.designer, args.string, dry_run)
...
def main():
    ...
    parser.add_argument(
        "-str",
        "--string",
        help="Override nameID=7 content with exact string (supersedes all other options)",
    )
