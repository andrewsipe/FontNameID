...
def process_file(
    ...
    if ext == ".ttx":
        return process_ttx_file(
            filepath, vendor_url, string_override, dry_run, empty_fields_only
        )
    else:
...
def process_files(file_paths, script_args, batch_context=False):
    ...
    if getattr(script_args, "empty_fields_only", False):
        operations.append("Only fill blank nameID 11 entries (--empty-fields-only)")
...
def main():
    ...
    parser.add_argument(
        ...
        help="Only fill blank Windows English nameID 11; do not overwrite existing text",
    )
...
class NameID11Replacer:
    ...
    name_id = 11
    description = "Vendor URL"
    supported_flags = {"vendor_url", "string", "empty_fields_only"}
