#!/usr/bin/env python3
"""
Font NameID 9 Replacer Script

Replaces the nameID="9" (Designer) record in font files.
...
def process_ttx_file(filepath, designer, string_override=None, dry_run=False):
    """Process TTX (XML) file to replace nameID="9" record"""
    try:
        if LXML_AVAILABLE:
        ...
        # Priority: string_override → designer
        new_name = string_override if string_override else designer

        # Find namerecord with nameID="9"
        namerecord_9 = name_table.find(
            './/namerecord[@nameID="9"][@platformID="3"][@platEncID="1"][@langID="0x409"]'
        )

        file_changed = False
        if namerecord_9 is not None:
            # Capture old value before updating
            old_text = namerecord_9.text.strip() if namerecord_9.text else ""
            if old_text == new_name:
                show_unchanged(9, filepath, old_text, dry_run, console)
            else:
                # Format text with proper indentation (newline + 6 spaces)
                namerecord_9.text = f"\n      {new_name}\n    "
                file_changed = True
                show_updated(9, filepath, old_text, new_name, dry_run, console)
        else:
            ...
            file_changed = True
            show_created(9, filepath, new_name, dry_run, console)
        ...
        if file_changed:
            ...
            deduplicate_namerecords_ttx(name_table, 9)
...
def process_binary_font(filepath, designer, string_override=None, dry_run=False):
    ...
    try:
        ...
        # Look for existing nameID=9 record with the specific platform/encoding
        found = False
        file_changed = False
        ...
        if file_changed:
            ...
            deduplicate_namerecords_binary(name_table, 9)
...
def process_file(filepath, designer, string_override=None, dry_run=False):
    ...
    if ext == ".ttx":
        return process_ttx_file(filepath, designer, string_override, dry_run)
    else:
        return process_binary_font(filepath, designer, string_override, dry_run)


def process_files(file_paths, script_args, batch_context=False):
    """
    Core processing logic for NameID9Replacer.
    ...
    if script_args.string:
        operations.append(
            f"Replace nameID 9 (Designer) with exact string: '{script_args.string}'"
        )
    else:
        designer = script_args.designer or "user input"
        operations.append(f"Replace nameID 9 (Designer) with: {designer}")
...
def main():
    ...
    sys.argv = _preprocess_explicit_syntax(sys.argv, 9)

    parser = argparse.ArgumentParser(
        description="Replace nameID='9' (Designer) records in font files",
    ...
    parser.add_argument(
        "-str",
        "--string",
        help="Override nameID=9 content with exact string (supersedes all other options)",
    )
